...
pub async fn subscribe(pool: web::Data<PgPool>, form: web::Form<SubscribeForm>) -> HttpResponse {
    ...
    let mut token = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789".to_string().into_bytes();
    ...
    if models::newsletters::subscribers::add(transaction.deref_mut(), &form.email, &token[..60]).await.is_err() {
...
pub async fn confirm_subscription(pool: web::Data<PgPool>, query: web::Query<ConfirmSubscriptionForm>) -> HttpResponse {
    #[derive(Template)]
